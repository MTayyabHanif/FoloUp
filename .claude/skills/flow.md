---
name: flow
description: Walk a change request through a consistent OpenSpec-backed pipeline. Use for any non-trivial change — bugs, features, refactors. The skill announces the planned shape, lets the user skip or expand steps, and delegates to OpenSpec, gstack, and code-context-analysis. Does NOT enforce, classify into tracks, or run silently — every invocation prints the plan and waits for confirmation.
---
...
Steps not enabled in `.claude/flow.yaml` are skipped by default. The user can force any step inline at the announcement gate.
...
- **No reimplementation.** Delegates to OpenSpec (`/opsx:*`), gstack (`~/.claude/skills/gstack/<name>/SKILL.md`), and code-context-analysis. Reads their files and follows them in-session.
...
> **Note on examples:** Concrete examples in this skill (request phrases, change names, scope paths) reference whatever is configured in the active project's `.claude/flow.yaml`. When you copy this skill to a new repo, those examples will read against your new `flow.yaml` keys, not these.
...
Read `.claude/flow.yaml`. If absent, use built-in defaults (pipeline as listed above, all `tools` true, empty `cgc_keyword_map`, empty `bypass.typo_patterns`) and announce that defaults are being used. Offer to write a starter `flow.yaml` once the user confirms it's wanted.

If `.claude/flow.local.yaml` exists, overlay it on top of `flow.yaml` via deep merge (per-key override; nested keys merged, arrays replaced, not concatenated). Announce when local overrides change a pipeline step, tool flag, or step model/dispatch (e.g., "`steps.qa.model: opus` from local overrides — qa will run on Opus instead of the default Sonnet").
...
If offering, surface via AskUserQuestion:
...
| Step | Check |
|---|---|
| `brainstorming` | `~/.claude/skills/gstack/office-hours/SKILL.md` exists |
| `cgc` | `~/.claude/skills/code-context-analysis/SKILL.md` exists |
| `propose` / `apply` / `archive` | `.claude/commands/opsx/<cmd>.md` exists |
| `plan_eng_review` | `~/.claude/skills/gstack/plan-eng-review/SKILL.md` exists |
| `plan_design_review` | `~/.claude/skills/gstack/plan-design-review/SKILL.md` exists |
| `qa` | `~/.claude/skills/gstack/qa/SKILL.md` exists |
| `review` | `~/.claude/skills/gstack/review/SKILL.md` exists |
| `ship` | `~/.claude/skills/gstack/ship/SKILL.md` exists |
...
For each INCLUDED step, also resolve its **model** (`sonnet` | `opus` | `haiku`) and **dispatch mode** (`subagent` | `main`) from `flow.yaml.steps.<step-name>`. If a step has no entry in `steps:`, default to `model: sonnet, dispatch: main` and surface a warning in the announcement. These two attributes drive Step 8's execution behavior.
...
Plan for `fix-hero-copy`:
  1. brainstorming       INCLUDED  (opus, subagent)    gstack:office-hours
  2. cgc                 INCLUDED  (sonnet, subagent)  scope: app/(marketing)/_components/hero/, rule: keyword-map:hero
  3. propose             INCLUDED  (opus, subagent)    opsx:propose
  4. plan_eng_review     INCLUDED  (sonnet, subagent)  gstack:plan-eng-review
  5. plan_design_review  SKIPPED-by-config
  6. apply               INCLUDED  (opus, main)        opsx:apply
  7. qa                  INCLUDED  (sonnet, subagent)  gstack:qa
  8. review              INCLUDED  (sonnet, subagent)  gstack:review
  9. ship                INCLUDED  (sonnet, subagent)  gstack:ship
 10. archive             INCLUDED  (haiku, main)       opsx:archive
```
...
- `model <step> <sonnet|opus|haiku>` — override the resolved model for one step (e.g., `model qa opus` to escalate a tricky test loop)
...
**Prompt template** (the /flow skill builds this and passes it as the Agent tool's `prompt`):
...
- `NEEDS_FOLLOW_UP` → present the open questions to the operator via AskUserQuestion. Operator answers. /flow re-dispatches the same step as a fresh subagent with the answers appended to the prompt under "**Operator follow-ups:**". Repeat until APPROVED, BLOCKED, or operator aborts. Max 3 follow-up rounds before auto-escalating to "Abort or proceed without resolution?"
- `BLOCKED` → surface the BLOCKED verdict to the operator verbatim. Do not advance. Operator decides whether to re-dispatch (possibly with `model <step> opus` to escalate), skip the step, or abort.

### Interactive steps (brainstorming, plan reviews) in subagent mode

Steps like `brainstorming`, `plan_eng_review`, and `plan_design_review` are interactive by nature — the underlying skills ask the operator clarifying questions. Subagent dispatch can't ask the operator directly (subagents have no AskUserQuestion access to the operator's terminal — they return a single message). The contract handles this via the `NEEDS_FOLLOW_UP` status:

- The subagent does its own reading/analysis, identifies the open questions, and returns them as a batch in the verdict.
- /flow asks the operator each question in sequence (using AskUserQuestion).
...
**Subagent dispatch fails or returns malformed verdict.** A subagent's verdict is malformed if it lacks the `## Status` line or has a status outside `APPROVED | NEEDS_FOLLOW_UP | BLOCKED`. Surface the raw subagent message to the operator verbatim, leave state unchanged, do not auto-retry. Operator decides whether to re-dispatch (possibly with a stronger model via `model <step> opus`) or fall back to main-context execution via `dispatch <step> main`.

**Subagent NEEDS_FOLLOW_UP loop exceeds 3 rounds.** Auto-escalate to AskUserQuestion: *"This step has gone 3 rounds of follow-ups without converging. Abort, switch to main-context execution, or escalate model to Opus?"* Operator decides.
...
This skill contains no project-specific values. All such values come from `.claude/flow.yaml`. To use this skill in a different project:

1. Copy `.claude/skills/flow.md` to the new repo.
2. Copy `.claude/flow.yaml` and edit `project`, `deploy`, and `cgc_keyword_map` for the new context.
3. Add `.claude/flow.local.yaml` to the new repo's `.gitignore`.
4. Run `/flow` on any change to verify the pipeline shape announces correctly.

**ClickUp integration.** To use ClickUp ingestion in a different project, ensure the ClickUp MCP server is connected in that workspace and set `integrations.clickup.enabled: true` in `.claude/flow.yaml` (the reserved block defaults to `false` for portability — operators opt in per repo). When the integration is disabled or the MCP tool is missing, ClickUp URLs in `/flow` requests cause a hard failure rather than silent fallback.
