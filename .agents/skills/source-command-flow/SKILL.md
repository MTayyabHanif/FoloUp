...
- If `.Codex/flow.yaml` is absent, use built-in defaults (full pipeline; all subagent steps Sonnet; main steps Opus for `apply` / Haiku for `archive`) and announce that defaults are being used.
