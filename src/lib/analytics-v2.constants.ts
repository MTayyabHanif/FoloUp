...
/**
 * `ANALYTICS_V2_ENABLED` — controls whether the webhook generates v2 analytics
 * at all. When `false`, behavior is identical to today (v1 only). Default: false.
 */
export function isAnalyticsV2Enabled(): boolean {
  const raw = process.env.ANALYTICS_V2_ENABLED;
  return raw === "true" || raw === "1";
}

/**
 * `ANALYTICS_V2_AS_PRIMARY` — when both flags are true, v2 is written to the
 * `analytics` column (the column the dashboard reads) and v1 is written to the
 * `analytics_v1` secondary column. When false during dual-write, v1 stays primary.
 * Default: false.
 *
 * **Snapshot rule** (per design.md): both flags MUST be read once at the top of
 * a webhook handler and not re-read mid-handler. Use `snapshotAnalyticsV2Flags()`
 * for that pattern.
 */
export function isAnalyticsV2AsPrimary(): boolean {
  const raw = process.env.ANALYTICS_V2_AS_PRIMARY;
  return raw === "true" || raw === "1";
}

export interface AnalyticsV2FlagSnapshot {
  v2Enabled: boolean;
  v2AsPrimary: boolean;
}

/**
 * Read both env flags once and return them as a frozen snapshot. Call at the
 * top of the webhook handler BEFORE the first `await`. Pass the snapshot into
 * downstream helpers — do NOT re-read `process.env` inside async helpers.
 */
export function snapshotAnalyticsV2Flags(): AnalyticsV2FlagSnapshot {
  return Object.freeze({
    v2Enabled: isAnalyticsV2Enabled(),
    v2AsPrimary: isAnalyticsV2AsPrimary(),
  });
}
