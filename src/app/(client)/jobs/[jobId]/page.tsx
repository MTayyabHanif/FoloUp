...
import { HeaderActions } from "../../interviews/[interviewId]/_components/header-actions";
import { SessionsPanel } from "../../interviews/[interviewId]/_components/sessions-panel";
...
export default function JobWorkspacePage({
  ...
  const [themeColor, setThemeColor] = useState("#203b14");
  const [railFilter, setRailFilter] = useState<"all" | WorkflowStage>("all");

  const selectedCallId = typeof searchParams.call === "string" ? searchParams.call : "";
  const isEditMode = searchParams.edit === true || String(searchParams.edit) === "true";

  const interviewer = interviewers.find((item) => item.id === interview?.interviewer_id) ?? null;
    ...
    const fetchData = async () => {
      try {
        ...
        const [interviewResponse, responsesResponse] = await Promise.all([
          getInterviewById(params.jobId),
          ResponseService.getAllResponses(params.jobId),
        ]);
  ...
  const handleSelectCandidate = async (candidate: WorkflowCandidate) => {
    router.push(`/jobs/${params.jobId}?call=${candidate.callId}`);
    ...
    try {
      await ResponseService.saveResponse({ is_viewed: true }, candidate.callId);
      setResponses((previousResponses) =>
        previousResponses.map((item) =>
          item.call_id === candidate.callId ? { ...item, is_viewed: true } : item,
        ),
      );
    } catch (error) {
      console.error(error);
  ...
  const handleToggle = async () => {
    try {
      ...
      await InterviewService.updateInterview({ is_active: updatedIsActive }, params.jobId);
  ...
  const handleThemeColorChange = async (newColor: string) => {
    try {
      await InterviewService.updateInterview({ theme_color: newColor }, params.jobId);

      setThemeColor(newColor);
      setInterview((previous) => (previous ? { ...previous, theme_color: newColor } : previous));
      ...
      <PageHeader
        ...
        actions={
          <HeaderActions
            ...
            onEdit={() => router.push(`/jobs/${params.jobId}?edit=true`)}
            onToggleActive={handleToggle}
            onPreview={seeInterviewPreviewPage}
            onOpenMarker={() => setShowColorPicker(true)}
            onManageInvites={() => router.push(`/jobs/${params.jobId}/invites`)}
          />
        }
      />
      ...
      <div className="grid gap-6 xl:grid-cols-[420px_minmax(0,1fr)]">
        ...
        <main className="min-w-0">
          {selectedCallId ? (
            ...
            <SummaryInfo
              workflow={workflow}
              onOpenCandidate={(callId) => router.push(`/jobs/${params.jobId}?call=${callId}`)}
            />
          )}
        </main>
      </div>
      ...
      >
        <div className="w-full max-w-sm rounded-[28px] bg-[#fbfdf6] p-1 text-[#0a1d08]">
          ...
          <p className="mt-2 text-sm leading-6 text-[#53614d]">
            This marker helps you recognize the workflow in recruiter surfaces only. It no longer
            changes the candidate experience.
          </p>
          <div
            role="radiogroup"
