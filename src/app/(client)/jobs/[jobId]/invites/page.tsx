...
import { ArrowLeft, Copy, Mail, RefreshCcw, Search, Trash2, Upload } from "lucide-react";
...
export default function JobInvitesPage({ params: paramsPromise }: Props) {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: getInterviewById is context-stable for this route loader.
  useEffect(() => {
    let cancelled = false;
    void (async () => {
      try {
        const result = await getInterviewById(params.jobId);
        if (!cancelled) {
          if (result) {
            setInterview(result);
          } else {
  ...
  const handleResend = async (invite: InviteRow) => {
    ...
    try {
      const res = await fetch(`/api/interviews/${interview.id}/invites/${invite.id}/resend`, {
        method: "POST",
      });
      if (!res.ok) {
        if (res.status === 409) {
  ...
  const handleRevoke = async (invite: InviteRow) => {
    ...
    try {
      const res = await fetch(`/api/interviews/${interview.id}/invites/${invite.id}`, {
        method: "DELETE",
      });
      if (!res.ok) {
        throw new Error(`status ${res.status}`);
    ...
    return (
      <PageShell>
        <PageHeader
          ...
          description="Return to the workspace and select an interview from the jobs page."
          actions={
            <Button
              variant="ghost"
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
        <main className="space-y-6">
          <div className="flex flex-col gap-4 rounded-[28px] border border-[#e0e5d5] bg-[#fbfdf6] p-5">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {(["pending", "reserved", "used", "expired", "revoked"] as InviteStatus[]).map(
                (key) => (
                  <FilterChip
                    key={key}
                    active={filter === key}
                ...
                <div className="flex flex-col items-center justify-center gap-2 px-4 py-12 text-center text-sm text-[#53614d]">
                  {invites.length === 0 ? (
                    <>
                      ...
                      <p className="font-medium text-[#0a1d08]">No invites sent yet</p>
                      <p className="max-w-sm">
                        Use the panel on the right to send the first invite. Each invite is
                        single-use, email-bound, and expires in 24 hours.
                      </p>
                    </>
                  ) : (
        ...
        <aside className="space-y-4">
          <div className="rounded-[28px] border border-[#e0e5d5] bg-[#fbfdf6] p-5">
            ...
            <h3 className="mt-2 text-xl font-semibold tracking-[-0.03em]">Invite one candidate</h3>
            <p className="mt-1 text-sm leading-6 text-[#53614d]">
              We&apos;ll email the candidate a personal link. Each invite is bound to the email you
              enter — the candidate must use the same address to start the interview.
            </p>
            <p className="mt-3 rounded-[16px] border border-[#e0e5d5] bg-[#f8faf3] px-3 py-2 text-xs leading-5 text-[#53614d]">
              Invites work regardless of invite-only mode. Enable invite-only in the job settings to
              require an invite.
            </p>
          ...
          <div className="rounded-[28px] border border-dashed border-[#c5ccb6] bg-[#f6f8ef] p-5">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <p className="text-sm font-semibold text-[#0a1d08]">Bulk import (coming soon)</p>
                <p className="mt-1 text-xs leading-5 text-[#53614d]">
                  Upload a CSV with names and emails to send dozens of invites at once. We&apos;ll
                  surface progress, failed rows, and a per-candidate status here.
                </p>
              </div>
            </div>
          </div>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              A candidate may be partway through entering the interview. Revoking will prevent them
              from starting if they haven&apos;t yet, but won&apos;t interrupt an active session.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
